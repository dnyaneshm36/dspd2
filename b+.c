...
status_code insert_betn_dbnodes(dblist_type *temp,dblist_type *newnode)
 {
  /*this code is add newnode next temp*/
...
btree_node* insert_record(btree_node * root ,dblist_type* newnode)
{
  ...
  /*  if(end==NULL&&ptr==root)
    ...
    else */
    {
      ...
      for(k=0;k<no_list_node&&done==0;k++)  //at most we are cheacking next  4 record from bottom list
      {
      ...
      light=insert_betn_dbnodes(ret_val,newnode); //this fun insert newnode after ret_val
